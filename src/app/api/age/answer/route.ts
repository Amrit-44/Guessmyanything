...
export async function POST(req: NextRequest) {
  try {
    ...
    const { sessionId, answer } = body as { sessionId?: string; answer?: AgeAnswer };
    if (!sessionId || !answer || !VALID.includes(answer)) return NextResponse.json({ error: "Invalid" }, { status: 400 });
    return NextResponse.json(await answerAgeQuestion(sessionId, answer));
