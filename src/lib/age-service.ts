...
async function buildSnapshot(session: any): Promise<AgeSnapshot> {
  const history = parseHistory(session.history);
  const confidence = session.confidence ?? computeAgeConfidence(session.minAge, session.maxAge);
  const estimatedAge = Math.round((session.minAge + session.maxAge) / 2);
  let question: AgeSnapshot["question"] = null;
  if (session.status === "playing" && history.length > 0) {
    const last = history[history.length - 1];
    question = { questionId: last.questionId, text: last.questionText, questionNumber: history.length, category: last.category };
  }
  let guess: AgeSnapshot["guess"] = null;
  if (session.finalGuess !== null && session.finalMin !== null && session.finalMax !== null) {
    guess = { age: session.finalGuess, min: session.finalMin, max: session.finalMax, confidence: session.confidence ?? 0 };
  }
  return { sessionId: session.id, status: session.status, question, range: { min: session.minAge, max: session.maxAge }, estimatedAge, questionCount: session.questionCount, history: history.map(h => ({ questionText: h.questionText, answer: h.answer, tag: h.tag })), confidence, guess };
}
...
export async function answerAgeQuestion(sessionId: string, answer: AgeAnswer): Promise<AgeSnapshot> {
  ...
  const result = applyAgeAnswer(minAge, maxAge, current);
  minAge = result.min; maxAge = result.max;
  ...
  if (q) {
    ...
    await db.ageQuestion.update({ where: { id: current.questionId }, data: { avgInfoGain: newAvg } });
  }
...
export async function confirmAgeGuess(sessionId: string, correct: boolean): Promise<AgeSnapshot> {
  ...
  if (correct) {
    const askedIds = parseAskedIds(session.askedIds);
    await db.ageQuestion.updateMany({ where: { id: { in: askedIds } }, data: { successCount: { increment: 1 } } });
    await db.ageSession.update({ where: { id: sessionId }, data: { status: "won" } });
...
export async function learnAgeFromFailure(sessionId: string, actualAge: number): Promise<{ ok: true }> {
  ...
  const recentResult = await db.ageResult.findFirst({ where: { guessedAge: session.finalGuess ?? 0 }, orderBy: { createdAt: "desc" } });
  if (recentResult) await db.ageResult.update({ where: { id: recentResult.id }, data: { actualAge } });
  await db.ageSession.update({ where: { id: sessionId }, data: { status: "lost" } });
  return { ok: true };
}
